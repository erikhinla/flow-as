...
# Usage:
#   ./scripts/deploy_remote.sh <ssh_target> [remote_dir] [--hermes-solo|--agents-only|--full-stack] [--no-pull]
#
# Examples:
...
if [[ $# -lt 1 ]]; then
  echo "Usage: $0 <ssh_target> [remote_dir] [--hermes-solo|--agents-only|--full-stack] [--no-pull]"
  exit 1
fi
