    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">Real agent runtimes. Output stays here for review before any external action.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        {(['alpha', 'beta', 'gamma'] as const).map((role) => {
          ...
          return <div key={role} className="rounded-lg border border-gray-200 bg-white p-4">
            <div className="flex items-center justify-between"><div><h3 className="text-lg font-semibold text-gray-950">{agent?.name || role}</h3><p className="text-sm text-gray-600">{role === 'alpha' ? 'Alpha · reputation work' : role === 'beta' ? 'Beta · production work' : 'Gamma · approval-gated work'}</p></div><span className={`h-3 w-3 rounded-full ${agent?.healthy ? 'bg-emerald-500' : 'bg-red-500'}`} /></div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[420px_1fr]">
        <form onSubmit={submitTask} className="rounded-lg border border-gray-200 bg-white p-4">
          <h3 className="text-lg font-semibold text-gray-950">Run an agent task</h3>
          <p className="mt-1 text-sm text-gray-600">The Canon is supplied to the assigned agent. This does not publish, schedule, or change an external account.</p>
          <div className="mt-4 space-y-4">
            ...
            <select value={form.risk_tier} onChange={(event) => setForm({ ...form, risk_tier: event.target.value })} className="h-10 w-full rounded-md border border-gray-300 px-3 text-sm">
              <option value="reputation">Hermes Agent · Alpha review</option><option value="time_loss">OpenClaw · Beta production</option><option value="downtime_security_money">Agent Zero · Gamma approval required</option>
            </select>
