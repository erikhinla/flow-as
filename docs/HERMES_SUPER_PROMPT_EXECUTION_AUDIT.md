...
- Root `docker-compose.yml` is present in this checkout; next step is verification against current runbooks and env requirements.
...
1. Establish deployment authority doc + branch rules in-repo (single source of truth): `docs/LAUNCH_AUTHORITY.md`.
...
8. Execute ecosystem diagnostic spine task: `runtime/queues/pending/ecosystem-diagnostic-spine-gamma.json`.
...
## How to get agents to deploy FLOW Agent AS (operator runbook)

### 0) Preconditions
- Deployment authority is set to Docker + Hostinger runtime path.
- Secrets are loaded in `.env` on the target host.
- Stack files are present on host (`docker-compose.yml`, repo checkout, scripts).

### 1) Bring the stack up on the target host
```bash
cd /opt/flow-agents
...
curl -s http://localhost:18000/v1/flow/health | jq
curl -s http://localhost:18000/v1/intake/status | jq
curl -s http://localhost:50090/v1/health | jq
```

### 3) Submit a bounded deployment task envelope to OpenClaw intake
Use this to route deployment work through the governed flow:
```bash
curl -s -X POST http://localhost:18000/v1/intake/task \
  -H 'Content-Type: application/json' \
  -d '{
    "task_id": "deploy-flow-as-001",
    "created_at": "2026-04-25T00:00:00Z",
    "source": "manual",
    "title": "Deploy FLOW Agent AS stack on production host",
    "goal": "Build and start compose services, then verify required health endpoints",
    "task_type": "implementation",
    "risk_tier": "high",
    "preferred_owner": "agent_zero",
    "inputs": {
      "host_path": "/opt/flow-agents",
      "required_checks": [
        "/v1/health",
        "/v1/flow/health",
        "/v1/intake/status"
    ...
    "output_required": "Deployment log + endpoint verification + rollback notes",
    "review_required": true,
    "rollback_required": true
  }' | jq
```

### 4) Monitor queue status and review gate
```bash
curl -s http://localhost:18000/v1/intake/queues/status | jq
```
- High-risk deployment tasks should be routed to `agent_zero` and remain review-gated.

### 5) Execute approved deployment action and capture artifacts
- Required artifacts before production mutation:
  - `task.diff`
  - `task.review.md`
  - `task.rollback.md`
- After approval, execute deploy command and record logs.

### 6) Post-deploy verification and rollback readiness
```bash
curl -s http://localhost:18000/v1/health | jq
curl -s http://localhost:18000/v1/flow/health | jq
docker compose logs --tail=100 bizbrain-lite
docker compose logs --tail=100 openclaw
docker compose logs --tail=100 agent-zero
docker compose logs --tail=100 hermes
```
- If health checks fail, execute rollback path immediately and mark task as blocked with cause.

## Proof-of-work protocol (anti-hallucination / anti-fabrication)

This is the control to prevent “coherent answer” fraud and require verifiable execution.

### Non-negotiable evidence for any completion claim
1. **File evidence**
   - Exact repo-relative file paths changed.
   - `git diff --name-only` output included.
   - `git show --name-only --stat <commit_sha>` included.
2. **Commit evidence**
   - Commit SHA(s) for each claimed implementation step.
   - Branch name and `git status --short` output.
3. **Execution evidence**
   - Exact commands run (copy/paste command line).
   - Result snippets for each command (pass/fail output).
4. **Runtime evidence (if deployment claimed)**
   - `docker compose ps` output.
   - Health endpoint outputs (`/v1/health`, `/v1/flow/health`, `/v1/intake/status`).
   - Relevant service logs (`bizbrain-lite`, `openclaw`, `agent-zero`, `hermes`).
...
- If any claim lacks matching evidence above, mark it as **UNPROVEN**.
- Two consecutive unproven claims by an agent = remove agent from production execution lane until re-validated.

### Acceptance gate for “done”
- “Done” is valid only when:
  - work is present in file system paths
  - work is anchored to commit SHA(s)
  - operator can reproduce verification commands successfully
  - outputs are attached to task record/artifacts (`task.diff`, `task.review.md`, `task.rollback.md`)

### 1) Authority and deployment decisions (required)
- Production Git branch: `main`.
- Merge gate: PR approvals are not required. Best-practice default remains feature branch -> PR -> passing checks -> merge to `main`; direct `main` changes should be reserved for urgent operator-approved fixes.
...
9. **Objective:** build shared ecosystem diagnostic spine.  
   **Artifact:** `DiagnosticEngine` implementation plan or code change set using `docs/EXECUTABLE_DIAGNOSTIC_SYSTEM_LAYOUT.md` and `runtime/queues/pending/ecosystem-diagnostic-spine-gamma.json`.  
   **Dependency:** steps 1-3; source repo references require network-enabled inspection.  
   **Done condition:** TransformBy10X Digital Fog, BizBuilders AI Custom Roadmap, and BizBot Mktng activation routing share one diagnostic architecture with no dead-end CTAs.

10. **Objective:** launch TransformBy10X B2C funnel.  
   **Artifact:** live routes for Fog Diagnostic + Fog Lift Kit.  
   **Dependency:** steps 4-9.  
   **Done condition:** end-to-end diagnostic-to-purchase path works and is tracked.

11. **Objective:** launch BizBuilders AI diagnostic surface.  
    **Artifact:** 15-question B2B diagnostic route + routing automation.  
    **Dependency:** steps 4-9 and TransformBy10X progression hooks.  
    **Done condition:** qualified leads route into infrastructure conversation queue.

12. **Objective:** launch BizBot Mktng paid assets.  
    **Artifact:** RevAnew and Reddit Marketing Guide product + checkout + delivery pages.  
    **Dependency:** steps 6-9.  
    **Done condition:** $97 offer purchase/delivery + attribution verified.

13. **Objective:** launch social asset operating loop.  
    **Artifact:** platform asset packs + CTA routing sheet + posting runbook.  
    **Dependency:** steps 10-12.  
